...
public class BasicDocument extends Document 
{
	...
	public int getNumSyllables()
	{
		...
		for(String word:textWords)
		{
		    if (word.toLowerCase().charAt(word.length()-1) == 'e') {
		        if (isMoreVowelsExist(word.substring(0, word.length()-1).toLowerCase()))
		        {
		            count+=countSylabbusInNormalWord(word.substring(0, word.length()-1).toLowerCase());
		        } 
		        else
		        {
		            count++;
		    ...
		    {
		        count+=countSylabbusInNormalWord(word.toLowerCase());
		    }
		}
	    return count;
	}
	
	private boolean isMoreVowelsExist(String word) {
	    Pattern splitter = Pattern.compile(VOWELTS_EXSIT_EXPRESSION);
	...
	public static void main(String[] args)
	{
		...
		testCase(new BasicDocument("i, challenge, you, to, 2.5, battle, of!? wits"),
		 8,7,3);
		testCase(new BasicDocument("This is a sentence."),
				 5,4,1);
		System.out.println("********************");
